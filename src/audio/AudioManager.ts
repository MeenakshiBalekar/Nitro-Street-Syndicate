...
// Procedural audio engine. The vertical slice ships no audio asset files, so the
// "premium" engine/nitro/SFX layer is fully synthesized with the Web Audio API.
// This is audible on web (the primary preview surface). On native there is no
// Web Audio, so this degrades to a safe no-op — wire expo-audio + samples there.
...
type Sfx = 'ui' | 'countdown' | 'go' | 'crash' | 'finish' | 'nitro';
...
class AudioManagerImpl {
  ...
  init(): void {
    // Lazy — the context is actually created on the first user gesture (unlock).
  }

  setEnabled(v: boolean): void {
    this.enabled = v;
    if (!v) this.stopEngine();
    if (this.master && this.ctx) this.master.gain.value = v ? 0.5 : 0;
  }

  // Browsers block audio until a user gesture; call this from taps/buttons.
  unlock(): void {
  ...
  private ensure(): Maybe<AudioContext> {
    if (!isWeb || !this.enabled) return null;
    if (!this.ctx) {
  ...
  // Called every frame during a race. ratio = speed/topSpeed (0..1).
